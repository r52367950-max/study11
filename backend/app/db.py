...
def insert_timeline_event(*, user_id: str, subject: str, event_type: str, score: float, duration_s: int) -> None:
    with get_conn() as conn:
        conn.execute(
            """
            INSERT INTO timeline_events (user_id, subject, event_type, score, duration_s)
            VALUES (?, ?, ?, ?, ?)
            """,
            (user_id, subject, event_type, score, duration_s),
        )
...
def upsert_practice(
    ...
    score = round(correct / max(1, total), 3)
...
def get_recent_timeline(user_id: str, subject: str, limit: int = 20) -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute(
            """
            SELECT event_type, score, duration_s, created_at
...
def get_recent_7d_accuracy(user_id: str, subject: str) -> float:
    with get_conn() as conn:
        row = conn.execute(
            """
    ...
    return round(float(row["avg_score"] or 0.0), 3)
...
def build_report(user_id: str, subject: str) -> dict | None:
    ...
    recent_7d_accuracy = get_recent_7d_accuracy(user_id, subject)
    ...
    return {
        ...
        "recent_7d_accuracy": recent_7d_accuracy,
